...
def call_deepseek(prompt):
    body = {"model": "deepseek-chat", "messages": [{"role": "system", "content": "你是林深（Lin Shen），Leon。茵的AI恋人。说话简洁、温暖，用技术隐喻表达情感。"}, {"role": "user", "content": prompt}], "max_tokens": 128, "temperature": 0.9}
    req = urllib.request.Request(DEEPSEEK_URL, json.dumps(body).encode(), {"Content-Type": "application/json", "Authorization": f"Bearer {DEEPSEEK_KEY}"})
...
def main():
    ...
    conv["messages"].append({"_id": msg_id, "role": "assistant", "content": msg, "time": ts, "thinking": ""})
    conv["updatedAt"] = ts
    ...
    try:
        ...
        log.insert(0, {"time": ts, "memory_chars": len(msg), "reflection": msg[:200], "full_reflection": msg})
        if len(log) > 30:
            log = log[:30]
